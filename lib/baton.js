...
export function mountBaton(siteConfig) {
  ...
  // shows the card and returns pending immediately. A WebMCP call cannot stay
  // open while the operator considers a page-level confirmation.
  async function confirmInPage(client, message, toolName, input) {
    ...
    if (hasRUI) {
      try {
        ...
        debug('requestUserInteraction unavailable — falling back to the page card: ' + (e && e.message));
      }
    }

    showConfirmCard(message, key);
    const status = 'pending';
    const waited = Date.now() - started;
    debug('requestUserInteraction unavailable (fallback card shown) — ' + toolName + ' waited ' + waited + 'ms → ' + status);
